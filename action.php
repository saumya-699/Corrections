	...
	if(isset($_POST["addToCart"])){
       ...
       $result = $stmt->get_result();
		...
		if($count > 0){
			...
			VALUES (?,?,?,?)";
			$stmt = $conn->prepare($sql); 
           $stmt->bind_param("ssss",$p_id,$ip_add,$user_id,1);
            $stmt->execute();
			...
			$sql = "SELECT id FROM cart WHERE ip_add = ? AND p_id = ?' AND user_id = ?";
			$stmt = $conn->prepare($sql); 
            $stmt->bind_param("sss",$ip_add,$p_id,-1);
            $stmt->execute();
			...
			VALUES (?,?,?,?)";
			$stmt = $conn->prepare($sql); 
            $stmt->bind_param("ssss", $p_id,$ip_add,-1,1);
            $stmt->execute();
...
if (isset($_POST["removeItemFromCart"])) {
	...
	if (isset($_SESSION["uid"])) {
		...
		$stmt = $conn->prepare($sql); 
        $stmt->bind_param("ss", $remove_id,$_SESSION[uid]);
        $stmt->execute();
	}else{
		$sql = "DELETE FROM cart WHERE p_id = ? AND ip_add = ?";
		$stmt->bind_param("ss", $remove_id,$ip_add);
        $stmt->execute();
	}
...
if (isset($_POST["updateCartItem"])) {
	...
	$qty = $_POST["qty"];
	if (isset($_SESSION["uid"])) {
		$sql = "UPDATE cart SET qty=? WHERE p_id = ? AND user_id = ?";
		$stmt = $conn->prepare($sql); 
        $stmt->bind_param("sss", $qty,$update_id,$_SESSION[uid]);
        $stmt->execute();
	}else{
		$sql = "UPDATE cart SET qty= ? WHERE p_id = ? AND ip_add = ?";
		$stmt = $conn->prepare($sql); 
        $stmt->bind_param("sss", $qty,$update_id,$ip_add);
        $stmt->execute();
	}
	if(mysqli_query($con,$sql)){
